...
class TensorboardWriter:
    ...
    def __init__(self, log_dir: Optional[str] = None, enabled: bool = False):
        ...
        if enabled:
            ...
            if not succeeded:
                message = (
                    "Warning: visualization (Tensorboard) is configured to use, but "
                    "currently not installed on this machine. Please install "
    ...
    def __getattr__(self, name: str) -> Callable:
        if name in self.tb_writer_ftns:
            ...
            return wrapper
        # default action for returning methods defined in this class, set_step() for
        # instance
        else:
            try:
