...
def prepro_doc(
    ...
    words_per_each_sentence = (
        torch.LongTensor(words_per_each_sentence).unsqueeze(0).to(device)
    )  # (1, n_sentences)
    ...
    encoded_doc = list(
        map(
            ...
            + [0] * (word_limit_per_sentence - len(s)),
            doc,
        )
...
def prepro_sent(
    ...
    encoded_sent = list(map(lambda w: word_map.get(w, word_map["<unk>"]), sentence)) + [
        0
    ] * (word_limit - len(sentence))
...
def classify(
    ...
    if model_name in ["han"]:
        # preprocess document
        encoded_doc, sentences_per_doc, words_per_each_sentence = prepro_doc(
            text, word_map
        )
        # run through model
        scores, word_alphas, sentence_alphas = model(
            encoded_doc, sentences_per_doc, words_per_each_sentence
    ...
    prediction = "Category: {category}, Probability: {score:.2f}%".format(
        category=rev_label_map[prediction.item()], score=score.item() * 100
    )
    return prediction
...
if __name__ == "__main__":
    text = (
        "How do computers work? I have a CPU I want to use. But my keyboard and "
        "motherboard do not help.\n\n You can just google how computers work. "
        "Honestly, its easy."
    )
    # text = (
    #   "But think about it! It\'s so cool. Physics is really all about math. what "
    #   "feynman said, hehe"
    # )
    # text = (
    #   "I think I'm falling sick. There was some indigestion at first. But now a "
    #   "fever is beginning to take hold."
    # )
    # text = (
    #   "I want to tell you something important. Get into the stock market and "
    #   "investment funds. Make some money so you can buy yourself some yogurt."
    # )
    # text = (
    #   "You know what's wrong with this country? republicans and democrats. always at "
    #   "each other's throats\n There's no respect, no bipartisanship."
    # )

    # load model and word map
    model, model_name, _, dataset_name, word_map, _ = load_checkpoint(
