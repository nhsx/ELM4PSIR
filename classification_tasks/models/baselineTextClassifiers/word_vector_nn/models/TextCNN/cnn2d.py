...
class TextCNN2D(nn.Module):
    ...
    def set_embeddings(
        self, embeddings: torch.Tensor, layer_id: int = 1, fine_tune: bool = True
    ) -> None:
        """
        ...
        if embeddings is None:
            ...
            if layer_id == 1:
                self.embedding1.weight = nn.Parameter(
                    embeddings, requires_grad=fine_tune
                )
            else:
                self.embedding2.weight = nn.Parameter(
                    embeddings, requires_grad=fine_tune
                )
    ...
    def forward(
        ...
        """
        # word embedding
        embeddings = self.embedding1(text).unsqueeze(
            1
        )  # (batch_size, 1, word_pad_len, emb_size)
        # multichannel
        if self.embedding2:
            embeddings2 = self.embedding2(text).unsqueeze(
                1
            )  # (batch_size, 1, word_pad_len, emb_size)
            embeddings = torch.cat(
                (embeddings, embeddings2), dim=1
            )  # (batch_size, 2, word_pad_len, emb_size)

        # conv
        conved = [
            self.relu(conv(embeddings)).squeeze(3) for conv in self.convs
        ]  # [(batch size, n_kernels, word_pad_len - kernel_sizes[n] + 1)]

        # pooling
        pooled = [
            F.max_pool1d(i, i.size(2)).squeeze(2) for i in conved
        ]  # [(batch size, n_kernels)]

        # flatten
        flattened = self.dropout(
            torch.cat(pooled, dim=1)
        )  # (batch size, n_kernels * len(kernel_sizes))
