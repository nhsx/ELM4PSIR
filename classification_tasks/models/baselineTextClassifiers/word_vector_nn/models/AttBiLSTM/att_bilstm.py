...
class AttBiLSTM(nn.Module):
    ...
    References
    ----------
    1. "`Attention-Based Bidirectional Long Short-Term Memory Networks for Relation
    Classification.
    <https://www.aclweb.org/anthology/P16-2034.pdf>`_"
    ...
    def forward(
        self, text: torch.Tensor, words_per_sentence: torch.Tensor
    ) -> torch.Tensor:
        """
        Parameters
        ----------
        ...
        )  # a PackedSequence object, where 'data' is the flattened words
        # (n_words, emb_size)

        # run through bidirectional LSTM (PyTorch automatically applies it on the
        # PackedSequence)
        rnn_out, _ = self.BiLSTM(
            packed_words
        )  # a PackedSequence object, where 'data' is the output of the LSTM
        # (n_words, 2 * rnn_size)
        # print(f"Shape of rnn_out after passing to BiLSTM first: {rnn_out}")
        # unpack sequences (re-pad with 0s, WORDS -> SENTENCES)
        rnn_out, _ = pad_packed_sequence(
            rnn_out, batch_first=True
        )  # (batch_size, word_pad_len, 2 * word_rnn_size)
        # print(f"Shape of rnn out after pad pack sequence is: {rnn_out.shape}")
        # eq.8: h_i = [\overrightarrow{h}_i ⨁ \overleftarrow{h}_i ]
        # H = {h_1, h_2, ..., h_T}
        # print(f"Shape of first half of rnn out to sum:
        # {rnn_out[ :, :, : self.rnn_size].shape}
        # and 2nd: {rnn_out[ :, :, self.rnn_size : ].shape}")
        # print(
        #   (
        #       "Shape of them added: "
        #       f"{(rnn_out[:,:,:self.rnn_size] + rnn_out[:,:,self.rnn_size:]).shape}"
        #   )
        # )
        H = (
            rnn_out[:, :, : self.rnn_size] + rnn_out[:, :, self.rnn_size :]
        ...
        # print(f"after attention r shape is: {r.shape}")
        # eq.12: h* = tanh(r)
