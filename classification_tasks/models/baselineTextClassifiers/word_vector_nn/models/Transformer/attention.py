...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(
        ...
        # Q·K^T / sqrt(d_k)
        att = torch.matmul(
            Q / self.scale, K.transpose(2, 3)
        )  # (batch_size, n_heads, word_pad_len, word_pad_len)

        # mask away by setting such weights to a large negative number, so that they
        # evaluate to 0 under the softmax
        if mask is not None:
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        )  # (batch_size, word_pad_len, n_heads, d_k)
        ...
        context = (
            ...
            .contiguous()
            .view(batch_size, -1, self.d_k * self.n_heads)
        )  # (batch_size, word_pad_len, n_heads * d_k)
