...
def encode_and_pad(
    ...
    encoded_docs = list(
        map(
            ...
            + [[0] * word_limit] * (sentence_limit - len(doc)),
            input_docs,
        )
    )
    ...
    words_per_sentence = list(
        map(
            lambda doc: list(map(lambda s: len(s), doc))
            + [0] * (sentence_limit - len(doc)),
            input_docs,
        )
    )
...
def run_prepro(
    ...
    train_docs, train_labels, word_counter = read_csv(
    ...
    assert (
        len(encoded_train_docs)
        == len(train_labels)
        == len(sentences_per_train_document)
        == len(words_per_train_sentence)
    )
    ...
    assert (
        len(encoded_test_docs)
        == len(test_labels)
        == len(sentences_per_test_document)
        == len(words_per_test_sentence)
    )
    torch.save(
        {
